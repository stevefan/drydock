...
out=$($SSH "ws exec infra -- drydock-rpc RequestCapability type=SECRET scope.secret_name=anthropic_api_key" 2>&1)
...
# Release
rel=$($SSH "ws exec infra -- drydock-rpc ReleaseCapability lease_id=$lease_id" 2>&1)
