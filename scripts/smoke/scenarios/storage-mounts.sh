...
cleanup() {
    ...
    $SSH "drydock exec infra -- bash -lc 'export AWS_ACCESS_KEY_ID=\$(cat /run/secrets/aws_access_key_id) AWS_SECRET_ACCESS_KEY=\$(cat /run/secrets/aws_secret_access_key) AWS_SESSION_TOKEN=\$(cat /run/secrets/aws_session_token) AWS_DEFAULT_REGION=us-west-2 && aws s3 rm s3://$BUCKET/greeting.txt >/dev/null 2>&1; aws s3 rb s3://$BUCKET >/dev/null 2>&1'" || true
}
trap cleanup EXIT

# Prereq: infra drydock running with INFRA_PROVISION lease.
$SSH "drydock exec infra -- drydock-rpc RequestCapability type=INFRA_PROVISION 'scope.actions=[\"s3:*\"]'" >/dev/null || {
...
# Create drydock; drydock create blocks until state=running.
$SSH "drydock create $NAME" >/dev/null 2>&1 || { echo "FAIL: drydock create"; exit 1; }

# Assert marker visible via mount
got=$($SSH "drydock exec $NAME -- cat /mnt/check/greeting.txt 2>&1")
if [ "$got" != "$MARKER" ]; then
    echo "FAIL: expected '$MARKER', got '$got'"
    $SSH "drydock exec $NAME -- cat /tmp/storage-mounts.log 2>&1" | sed 's/^/  log: /'
    exit 1
fi
echo "marker read through mount: OK ($got)"

# Assert refresh daemon is alive (Phase C.1)
refresh_status=$($SSH "drydock exec $NAME -- bash -lc 'pid=\$(cat /tmp/storage-mounts-refresh.pid 2>/dev/null); if [ -n \"\$pid\" ] && kill -0 \$pid 2>/dev/null; then echo alive:\$pid; else echo dead; fi'")
