...
class TestEmitAudit:
    def test_writes_v2_shape(self, tmp_path):
        ...
        emit_audit(
            "drydock.created",
            principal="dock_alpha",
            request_id="req-1",
            method="CreateDesk",
            result="ok",
            details={"drydock_id": "dock_alpha", "project": "p", "parent_drydock_id": None},
            log_path=log,
    ...
    def test_appends_does_not_truncate(self, tmp_path):
        log = tmp_path / "audit.log"
        emit_audit("drydock.created", principal=None, request_id=None,
                   method="CreateDesk", result="ok", log_path=log)
        emit_audit("drydock.destroyed", principal=None, request_id=None,
    ...
    def test_unknown_event_raises(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="unknown audit event"):
            emit_audit("drydock.exploded", principal=None, request_id=None,
    ...
    def test_request_id_int_coerced_to_string(self, tmp_path):
        log = tmp_path / "audit.log"
        emit_audit("drydock.created", principal=None, request_id=42,
                   method="CreateDesk", result="ok", log_path=log)
        entry = json.loads(log.read_text().strip())
...
class TestV1Coexistence:
    def test_log_event_and_emit_audit_share_file(self, tmp_path):
        log = tmp_path / "audit.log"
        log_event("drydock.created", "dock_alpha", log_path=log)
        emit_audit("drydock.created", principal="dock_alpha", request_id="r1",
                   method="CreateDesk", result="ok", log_path=log)
        ...
        assert lines[0]["event"] == "drydock.created"
        assert "drydock_id" in lines[0]
        assert lines[1]["event"] == "drydock.created"
