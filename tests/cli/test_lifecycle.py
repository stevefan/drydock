...
# Regression: suspended drydocks couldn't be resumed — DrydockExistsError
@patch("drydock.cli.create.DevcontainerCLI")
def test_create_on_suspended_drydock_resumes(MockCLI, tmp_path, monkeypatch):
    """Catches failure where suspended drydocks can't be resumed via ws create."""
    monkeypatch.setenv("HOME", str(tmp_path))
    repo = tmp_path / "repo"
    _init_repo(repo)

    # Set up a suspended drydock with an existing checkout
    registry = Registry(db_path=tmp_path / ".drydock" / "registry.db")
    checkout_dir = tmp_path / ".drydock" / "worktrees" / "dock_myws"
    subprocess.run(
    ...
    refreshed = registry.get_drydock("myws")
...
def test_create_on_running_drydock_errors_with_fix(tmp_path, monkeypatch):
    ...
    assert err["error"] == "drydock_already_running"
    assert "fix" in err
...
def test_force_rebuild_preserves_checkout(MockCLI, tmp_path, monkeypatch):
    ...
    registry = Registry(db_path=tmp_path / ".drydock" / "registry.db")
    checkout_dir = tmp_path / ".drydock" / "worktrees" / "dock_forcews"
    subprocess.run(
        ["git", "clone", str(repo), str(checkout_dir)],
        capture_output=True, text=True, check=True,
    )
    ...
    refreshed = registry.get_drydock("forcews")
    assert refreshed.state == "running"
    assert refreshed.container_id == "new-ctr-456"
    registry.close()


# Contract: error-state drydocks need --force to rebuild; non-forced create must
# not silently reuse them (which would ignore new CLI args and mask the failure).
def test_create_on_error_drydock_requires_force(tmp_path, monkeypatch):
    """Catches regression where error-state drydocks get silently reused on plain
    ...
    assert result.exit_code != 0
    err = json.loads(result.output.strip())
    assert err["error"] == "drydock_in_error_state"
    assert "--force" in err["fix"]
    registry.close()
...
def test_force_rebuild_preserves_named_volumes(MockCLI, tmp_path, monkeypatch):
    ...
    registry = Registry(db_path=tmp_path / ".drydock" / "registry.db")
    checkout_dir = tmp_path / ".drydock" / "worktrees" / "dock_volws"
    subprocess.run(
        ["git", "clone", str(repo), str(checkout_dir)],
        capture_output=True, text=True, check=True,
    )
...
def test_destroy_preserves_named_volumes(MockCLI, tmp_path, monkeypatch):
    ...
    registry = Registry(db_path=tmp_path / ".drydock" / "registry.db")
    checkout_dir = tmp_path / ".drydock" / "worktrees" / "dock_destws"
    subprocess.run(
        ["git", "clone", str(repo), str(checkout_dir)],
        capture_output=True, text=True, check=True,
    )
