...
On the auth Harbor, a `wsd`-managed timer fires every 4h:
...
1. User runs `ws auth login --on <auth-harbor>` from any device. CLI prints an Anthropic authorize URL with `redirect_uri=http://<auth-harbor>.tailnet:8443/oauth/callback`.
2. User opens URL on phone. Phone's browser, on tailnet, follows the Anthropic redirect to the auth Harbor.
3. Auth Harbor's listener (a small HTTPS endpoint served by `wsd`, or a sidecar) receives `?code=...`, exchanges with Anthropic for tokens, stores as `claude_master_refresh_token`.
...
**Critical unknown:** does Claude Code's OAuth client accept arbitrary `redirect_uri` values, or is it pinned to `http://localhost:<port>`? Three fallbacks if pinned:
  ...
  - **Code-paste:** phone visits authorize URL, gets code, user types into `ws auth submit-code <code> --on <auth-harbor>`. Always works; needs typing.
...
| Command | Purpose |
|---|---|
| `ws auth designate <harbor>` | Mark a Harbor as the auth Harbor for the current identity. Writes to local config. |
...
- `ws auth seed --from-keychain` is the only operation that exposes the refresh token in transit. Use SSH-over-tailnet, never plain network.
...
4. Migration: how do peers re-key when a refresh-token rotation happens? Current design assumes immediate push to all peers; consider a pull-on-demand variant where peers request fresh creds via `RequestCapability` and `wsd` mints them.
