...
A **fleet-monitor desk** runs on one designated Harbor (default: the auth Harbor — same machine, same trust, always-on). The desk:

- Has a `peers.yaml` listing each peer Harbor's tailnet hostname and the desks to monitor (or `*`).
...
| Per-desk deskwatch | `wsd.deskwatch_eval <desk>` | Job/output/probe violations |
...
| `ws fleet history <harbor> <desk>` | Time-series of probe results for one desk. |
...
- **Peer auth:** each peer Harbor's `wsd` issues a `fleet-monitor` bearer token (scope: read-only ping/list/deskwatch). Stored as `fleet_monitor_token_<peer>` in the monitor desk's secrets.
...
3. CC-liveness probe: does the access-token validation cost count against rate limits in a meaningful way at 60s × N desks? Probably negligible but check.
