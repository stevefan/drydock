...
**Purpose.** This is the canonical model for what each layer of drydock is, what owns what, and how the pieces relate. Every other doc points here; if a doc disagrees with this one, this one wins. **Last full revision: 2026-05-05** following the V3 architectural reframe (DryDock-as-harness, Authority+Auditor split with Harbormaster deferred, Yard layer).

Code identifiers were consolidated to drydock-vocabulary on 2026-05-06 (`Drydock` class, `drydocks` SQLite table, `dock_<slug>` ID prefix, `drydock` CLI binary, `drydock daemon` subcommand, `drydock.*` audit events, `daemon.*` RPC methods). The legacy `ws`/`wsd`/`workspace`/`desk` names are gone from the user-facing surface; the Registry runs an idempotent migration on first init that renames live data in place.
...
|---|---|---|---|
| 0 | **Archipelago** *(textual metaphor)* | The collection of Harbors a principal owns. Currently a small set of static island machines (Mac, Hetzner). | A perspective the principal holds. No daemon owns the archipelago; it's how we *describe* the harbors collectively. The technical/CLI term is just **Harbors** (`drydock harbors status`). |
| 1 | **Harbor** | The host machine running `drydock daemon`. An island in the archipelago. | The daemon, the registry, policy state, capability-broker leases, audit log, daemon-level admin secrets. One `drydock daemon` per Harbor. |
| 2a | **Harbor Authority** | The rule-enforcement role. The gate — capability broker, narrowness check, resource ceiling enforcement. | Auth, narrowness validation at every capability request, throttle/deny actions per standing policy, hard cgroup ceiling enforcement. Mostly what `drydock daemon` *already does* in V1/V2; just needs naming as a distinct role. |
...
|---|---|---|
| **Principal direct** | None (you're the principal) | Edit YAML, `drydock project edit auction-crawl`, git commit + reload |
| **Dockworker, within standing policy** | Authority auto-applies | Open a domain that matches `delegatable_network_reach` glob; register a workload within `workload_max`; request a secret already in `delegatable_secrets` |
| **Dockworker, novel** | Auditor escalates to principal | Request a new domain not covered by glob; request permanent ceiling lift; propose a new Ship in the Fleet; request a new capability type |

The Dockworker's amendment proposals carry structured metadata (the EGRESS_GRANTS prototype shape, generalized): `requested_by` service name, `reason` prose, `tos_notes`, `status: pending|approved|denied`, `reviewed_by/at/note` for review trail. The audit log records every proposal, every approval/denial, every application. Principal review happens via Telegram (one-word reply) or by editing the YAML directly.
...
- "Harbor `drydock-hillsboro` runs `drydock daemon`" — correct.
...
- "The auction-crawl Dockworker requested NETWORK_REACH for a new domain; Auditor escalated" — correct (amendment outside standing policy → escalation).
...
- "**desk**" — colloquial alias for what's now called **Dock**. Office-metaphor leak. Code identifiers (`drydock_id`, `deskwatch`, `delegatable_*`) remain frozen.
...
|---|---|---|
| Harbor | host running `drydock daemon`; no separate identifier | n/a |
| Harbor Authority | mostly `drydock daemon` itself today (capability handlers, narrowness validation, bearer-token auth) | the role mostly exists; just needs the name |
...
- Capability broker, audit emission, token issuance — all Harbor-level performed by `drydock daemon`. The DryDock is *subject* to these; it doesn't make them.
- One `drydock daemon` per Harbor. Multi-daemon single-host is out of scope.
...
**Bucket 1 — pure deterministic enforcement (Authority handles).** Capability gate, narrowness check, cgroup ceiling, lease expiry. No LLM; predictability + speed required. The Authority is mostly `drydock daemon` as it already exists.
...
- Proposes infrastructure amendments via broker (which the Authority auto-applies or the Auditor escalates).
...
- **V3** (2026-05-05 reframe, this doc): DryDock-as-harness vs Ship-as-runtime separation. Yard layer. Authority + Auditor split (Harbormaster as deferred manager role). Multi-Ship Fleets. Container = security boundary, DryDock = resource boundary. IaC + agent-amendment contract.

V3 is the conceptual model going forward. Implementation hasn't fully caught up — the code still has many V1/V2 shapes (one-container-per-Workspace, Project-as-YAML-not-tracked, Authority/Auditor surfaces not-yet-implemented). The implementation work is to migrate code to this model incrementally; the docs (this one and others) describe the *target* model so all future work points the same direction.
