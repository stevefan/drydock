...
StopDesk(drydock_id)
DestroyDesk(drydock_id, force?)                        (cascades to children)

# Introspection
ListDesks(filter?)                    → [DeskSummary]
ListChildren(parent_id)               → [DeskSummary]
InspectDesk(drydock_id)                  → DeskDetail
...
daemon.health                            → { ok, pid, version }
daemon.whoami                            → { drydock_id }
```
...
`scripts/drydock-rpc` is a single-file stdlib-only Python JSON-RPC client. `drydock host init` deploys it to `~/.drydock/bin/drydock-rpc`; the overlay bind-mounts that path read-only at `/usr/local/bin/drydock-rpc` inside every container.

```
$ drydock-rpc daemon.whoami
{ "drydock_id": "dock_auction_crawl" }

$ drydock-rpc RequestCapability \
    type=SECRET \
    scope.secret_name=anthropic_api_key
{ "lease_id": "ls_...", "drydock_id": "dock_auction_crawl", ... }
...
Env: `DRYDOCK_DAEMON_SOCKET` (default `/run/drydock/daemon.sock` inside containers; `~/.drydock/run/daemon.sock` on the Harbor). Token: read from `/run/secrets/drydock-token` and sent as the `auth` field of the JSON-RPC request.

### Harbor CLI

`drydock create`, `drydock stop`, `drydock destroy`, etc. detect `$DRYDOCK_WORKSPACE_ID` env. If set, they route to the daemon (drydock mode). If not, they write SQLite + call devcontainer CLI directly (Harbor mode). `_daemon_overlay_params()` in `src/drydock/cli/create.py` builds the RPC params dict from the project YAML.
...
- Issued at `CreateDesk` or `SpawnChild` (see `src/drydock/daemon/auth.py :: issue_token_for_desk`)
- Materialized at `~/.drydock/secrets/<dock_id>/drydock-token` on the Harbor, mode 0400, owner container-uid (1000). Bind-mount makes it visible at `/run/secrets/drydock-token` inside the container.
...
- Rotation reserved (`RotateDeskToken` admin method). For now: re-issue by `drydock destroy && drydock create`.

Token → `drydock_id`. Token does NOT carry capability claims; the daemon looks up the drydock's current capabilities on every request. That means:
...
Multi-user generalization is reserved: the map becomes `token → (principal_id, drydock_id)` when principals become explicit. Adding a principal later is additive; every audit consumer and policy-keyed store participates.
...
## `drydock attach` routes direct

`drydock attach` opens a local editor (VS Code, Cursor) connected to the drydock's container via the `vscode-remote+dev-container://` URL pattern. No policy happens there; daemon isn't in the path. In-drydock `drydock attach` (drydock → drydock) is not supported.
