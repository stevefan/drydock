...
**Soft ceilings** are observed by the deputy. Exceeding one triggers a deputy *action* (warn / throttle / revoke / stop / escalate per policy) but the substrate doesn't enforce them. Right tool for things where "exceeded" means "spending too much money or quota": Anthropic API tokens, AWS API calls, network egress bytes, lease hold time. The kernel doesn't know what an API token costs.

The split matters because they have different failure modes (substrate kill is abrupt and unsignalable; deputy action is slower but graceful) and different authoring semantics (cgroup limits are "what the substrate can enforce"; soft caps are "what the principal cares about budget-wise").
...
narrowness:
  ...
  resources:
    ...
    # Soft — deputy-observed in software
    soft:
    ...
    # deputy action when that ceiling is breached.
    on_violation:
...
Globals at `~/.drydock/policy/global.yaml` use the same shape. Per-desk *narrows*, never widens — schema validation rejects per-desk soft ceilings higher than global.
...
- `workspace_disk_max` — quota on the `/workspace` volume. Today this needs a quota-aware fs (xfs/btrfs); ext4 falls back to a periodic du-check by the deputy. Document the fallback as soft-on-ext4.

**Soft** (everything maps to something the deputy can count):
- `egress_bytes_per_day` / `egress_bandwidth_max` — bytes counted at the container's veth via `iptables -L -v` or `nftables` counters; the deputy reads on its poll cycle and computes deltas. Bandwidth ceiling is enforced via `tc/htb`.
...
- `aws_calls_per_hour` — counted from the AWS STS lease's CloudTrail-ish call log when issued via `RequestCapability(STORAGE_MOUNT)`; the broker already mints these and can attribute calls back to desks.
- `lease_hold_max` / `idle_lease_revoke_after` — broker-tracked since the broker mints leases; sweeper runs on the deputy's cadence.
...
- `stop` — `ws stop` the desk (preserves volumes).
- `escalate` — push to principal via Telegram, take no enforcement until response.

### Defaults the principal will inherit

A `defaults` section in `global.yaml` applies to any desk that doesn't override:
```yaml
defaults:
  hard:
    cpu_max: 2.0
...
Empty defaults = no ceilings (current behavior). Recommend the principal start with the above and tighten per-desk.

---

## 3. Workload registration

A worker about to burn meaningfully more than its standing soft caps **declares the workload first**. The deputy responds with a `WorkloadLease` that lifts the relevant caps for the lease window; without a registration, the same burn trips enforcement.
...
### Deputy decision logic

1. Look up effective policy for caller_desk (per-desk ∩ global).
...
### Container running
- Soft ceilings polled every N seconds (configurable; default 60s) by deputy via fleet-monitor + in-desk probe.
- Workload leases tracked in broker; expiry sweeper runs on deputy cadence.

### Container stop
- All workload leases for that desk auto-revoke.
...
- In-flight workload leases survive only if the container restart was deputy-initiated within the lease window (e.g., `RestartDeskAgent` for credential refresh); otherwise revoked.
...
- Soft ceiling diffs apply immediately on the deputy's next poll.
...
`workload.deviation` is the interesting one: the principal can run a "where do my desks consistently under-declare or over-declare?" report and tighten/loosen the standing caps accordingly. This is the feedback loop that keeps the model honest over time.
...
| Command | Purpose |
|---|---|
| `ws resources show <desk>` | Effective ceilings (per-desk ∩ global) and current usage. |
...
- No deputy action yet; no workload registration.
...
- In-desk probe (`desk-probe`, per principal-deputy-governance.md §5d) ships in drydock-base with a snapshot RPC.
- Deputy poll cycle: read snapshots, compute deltas, write to `resource_usage` table.
- `on_violation: warn` actions emit deskwatch violations.
- `ws resources history` works.

This makes consumption visible without enforcement. Calibration phase — the principal watches what their desks actually use before tightening.
...
3. **Honest-declaration incentive.** Nothing prevents a worker from declaring small workloads to evade enforcement. Mitigations: deviation tracking (consistent under-declaration triggers escalation), per-desk reputation (desks that frequently deviate get lower auto-grant ceilings). Probably not a V1 problem since the failure mode is "Steven notices his agents lying," not "adversary." Revisit if multi-user lands.
4. **Workload-against-deputy.** Can the deputy itself register workloads? Probably no — the deputy's resource budget is principal-set and not lifted by self-declaration; the principal-out-of-band-kill protocol is what bounds deputy resource use. Document explicitly so the model stays clean.
