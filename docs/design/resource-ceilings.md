...
**Status:** sketch · not yet implemented · **Depends on:** [capability-broker.md](capability-broker.md), [narrowness.md](narrowness.md), [harbor-authority.md](harbor-authority.md), [port-auditor.md](port-auditor.md), [deskwatch.md](deskwatch.md), [harbor-monitor.md](harbor-monitor.md)

This doc covers two adjacent primitives: the schema for *resource ceilings* a drydock operates under, and the *workload registration* primitive that lets desks legitimately burst above their standing caps without being mistaken for a runaway. Authority enforces the leases; Auditor observes for divergence.
...
**Soft ceilings** are observed by the Auditor. Exceeding one triggers an Auditor recommendation (warn / throttle / revoke / stop / escalate per policy) but the substrate doesn't enforce them. Right tool for things where "exceeded" means "spending too much money or quota": Anthropic API tokens, AWS API calls, network egress bytes, lease hold time. The kernel doesn't know what an API token costs.

The split matters because they have different failure modes (substrate kill is abrupt and unsignalable; Auditor-mediated action is slower but graceful) and different authoring semantics (cgroup limits are "what the substrate can enforce"; soft caps are "what the principal cares about budget-wise").
...
narrowness:
  ...
  resources:
    ...
    # Soft — Auditor-observed in software
    soft:
    ...
    # Auditor-recommended action when that ceiling is breached (enforced by Authority).
    on_violation:
...
- `workspace_disk_max` — quota on the `/workspace` volume. Today this needs a quota-aware fs (xfs/btrfs); ext4 falls back to a periodic du-check by the Auditor. Document the fallback as soft-on-ext4.

**Soft** (everything maps to something the Auditor can count):
- `egress_bytes_per_day` / `egress_bandwidth_max` — bytes counted at the container's veth via `iptables -L -v` or `nftables` counters; the Auditor reads on its poll cycle and computes deltas. Bandwidth ceiling is enforced via `tc/htb` by Authority.
...
- `lease_hold_max` / `idle_lease_revoke_after` — broker-tracked since the broker mints leases; sweeper runs on the Auditor's cadence.
...
- `stop` — `drydock stop` the drydock (preserves volumes).
...
A worker about to burn meaningfully more than its standing soft caps **declares the workload first**. The Authority issues a `WorkloadLease` that lifts the relevant caps for the lease window; without a registration, the same burn trips enforcement.
...
### Workload-grant decision logic (Authority)
...
### Container running
- Soft ceilings polled every N seconds (configurable; default 60s) by Auditor via harbor-monitor + in-desk probe.
- Workload leases tracked in broker; expiry sweeper runs on Auditor cadence.

### Container stop
- All workload leases for that drydock auto-revoke.
- Final deviation-from-declared recorded in audit.

### Container restart (`drydock stop && drydock create`)
...
- In-flight workload leases survive only if the container restart was Auditor-initiated within the lease window (e.g., `RestartDeskAgent` for credential refresh); otherwise revoked.

### Project YAML reload (`drydock project reload`)
...
- Soft ceiling diffs apply immediately on the Auditor's next poll.
...
```
{event: "resource.violation",  drydock_id, ceiling, value, limit, action_taken}
{event: "workload.registered", drydock_id, workload_id, kind, expected, granted}
{event: "workload.deviation",  drydock_id, workload_id, resource, declared, actual}
{event: "workload.expired",    drydock_id, workload_id, summary}
{event: "ceiling.lifted",      drydock_id, ceiling, from, to, by_workload, by_principal}
```
...
| Command | Purpose |
|---|---|
| `drydock resources show <desk>` | Effective ceilings (per-drydock ∩ global) and current usage. |
| `drydock resources history <desk> [--days N]` | Time series of usage vs ceilings; flags excursions. |
| `drydock workload list [<desk>]` | Active and recent workload leases. |
| `drydock workload register <desk> --kind K --description D ...` | Manually register from the Harbor (debug / one-off). |
| `drydock workload revoke <workload_id>` | Cut short a workload lease. |
| `drydock ceiling raise <desk> <ceiling> <value> --duration D` | Principal-initiated ceiling lift outside the workload mechanism. Audited as principal action. |

Worker side: `drydock-rpc RegisterWorkload kind=training description=... expected.cpu_hours=4 ...` once the daemon handler dispatches it.
...
- `drydock create` translates hard ceilings to docker flags (`--cpus`, `--memory`, `--pids-limit`).
- `drydock resources show <desk>` (read-only).
- No Auditor action yet; no workload registration.
...
- In-desk probe (`desk-probe`) ships in drydock-base with a snapshot RPC.
- Auditor poll cycle: read snapshots, compute deltas, write to `resource_usage` table.
- `on_violation: warn` actions emit deskwatch violations.
- `drydock resources history` works.
...
- Bidirectional Telegram for escalations (Auditor → principal).

Phase C is where the model becomes self-regulating; A and B are the substrate for it.

**Phase ordering.** Phase A (hard cgroup ceilings) is the only piece that can ship without the Authority RPC surface for Auditor-initiated actions being designed. Phase B requires the in-desk `desk-probe`. Phase C requires lease TTL + idle revocation in the broker, plus the Auditor↔Authority action channel.

**Where cgroup metrics come from.** Phase B's soft observation reads from the in-container `desk-probe` for process/fd/disk-growth visibility, and from the Harbor's cgroup interface for CPU/memory totals. [harbor-monitor.md](harbor-monitor.md) is *not* extended to gather cgroup stats — it stays focused on liveness/health/CC-token probes; the Auditor joins harbor-monitor's signal with desk-probe's snapshot to reach a verdict.
...
- **Workload kind taxonomy.** Closed enum for V1 (training/crawl/batch/experiment/interactive); principal can add custom kinds in `global.yaml`. Closed enum makes per-kind policy possible.
- **Honest-declaration incentive.** Not a V1 problem — the failure mode is "Steven notices his agents lying," not adversary. Revisit if multi-user lands.
- **Workload-against-Auditor.** No — the Auditor's resource budget is principal-set at create-time and not lifted by self-declaration; the out-of-band kill protocol is what bounds Auditor resource use.

**Still open:**

1. **Per-resource enforcement plane.** Egress bandwidth via `tc/htb` adds a Harbor-side dependency. Anthropic-token throttle requires worker cooperation (broker can revoke the `claude_credentials` lease, but in-memory tokens persist until process restart — so throttle = revoke + `RestartDeskAgent`). Document the cooperative-vs-coercive distinction per resource.
2. **Cost mapping.** `anthropic_tokens_per_day` is a proxy for spend, not spend itself. Future: a `daily_dollar_cap` that aggregates Anthropic token cost + AWS STS-lease-attributed cost + tailnet bandwidth cost into one number. Out of scope for Phase A–C.
