...
class CapabilityLease:
    ...
    type: CapabilityType               # SECRET | STORAGE_MOUNT | (reserved)
...
**`expiry` policy.** `SECRET` leases default to `None` — live until the caller calls `ReleaseCapability` or the drydock is destroyed. `STORAGE_MOUNT` leases always carry a concrete expiry (the STS session's own expiration, typically 4h). Finite-TTL + auto-renew machinery is reserved for backends that need it, not universal.
...
| Type | Status | Scope shape |
|---|---|---|
| `SECRET` | Shipped | `{secret_name, source_desk_id?}` |
| `STORAGE_MOUNT` | Shipped | `{bucket, prefix, mode}` |
...
- **`StsAssumeRoleBackend`** — calls `aws sts assume-role` against the configured `drydock-agent` role with an inline session policy constructed by `build_session_policy(bucket, prefix, mode)`. The long-lived `drydock-runner` IAM keys stay on the Harbor; the worker only ever sees the scoped session credential. Default session duration 4h.
- **`StubStorageBackend`** — deterministic fake credentials for tests and for Harbors without AWS wired up.

`build_session_policy` is a pure function — the rendering stays stable even as callers change. Mode vocabulary: `ro` (`GetObject` + `ListBucket`), `rw` (additionally `PutObject` + `DeleteObject`). Adding modes is additive.
...
| `STORAGE_MOUNT` | `aws_access_key_id`, `aws_secret_access_key`, `aws_session_token`, `aws_session_expiration` | `~/.drydock/secrets/<caller>/` |
...
- **`STORAGE_MOUNT`**: one active lease per drydock. On new issue, any prior active `STORAGE_MOUNT` lease for the caller is implicitly revoked with `revocation_reason = "superseded"` and a `lease.released` audit event. The four `aws_*` files are overwritten in place. This keeps release cleanup unambiguous — we always know which lease currently owns the files.

Revocation path: `registry.find_active_storage_lease()` inside `_handle_storage_request()` in `capability_handlers.py`.
...
| -32015 | `storage_backend_not_configured` | `STORAGE_MOUNT` requested but no backend in `wsd.toml` |
