...
> **Vocabulary note (2026-04-17):** Entries below predate the Harbor / DryDock / Worker product-vocabulary shift and are preserved verbatim as history. In current vocabulary: "host" (the machine running `drydock daemon`) is a **Harbor**, "agent-desk" / "workspace" (product concept) is a **DryDock**, and the agent running inside is a **Worker**. Code identifiers (`ws_<slug>`, `workspaces` table, `Workspace` class, CLI `ws` prefix) are unchanged. See [design/vocabulary.md](design/vocabulary.md).
...
- **Persistence pivot.** V3 cross-host migration archived (`docs/_archive/migration-vision.md`); drydocks are durable on a chosen Harbor. Reboot-resume via systemd units (`drydock.service` + `drydock-desks.service` with ExecStop hook). Resume-on-CreateDesk for suspended drydocks + ungraceful-shutdown detection.
- **In-desk RPC.** `drydock daemon` socket bind-mounted into every drydock at `/run/drydock/daemon.sock` (directory bind — durable across daemon restart). Stdlib-only client `drydock-rpc` bind-mounted at `/usr/local/bin/drydock-rpc`. Socket chmod 0o666 so non-root workers can connect; bearer-token auth remains the real security gate.
- **V4 Phase 1: STORAGE_MOUNT.** New `CapabilityKind.REQUEST_STORAGE_LEASES`, `[storage]` daemon.toml section, `StsAssumeRoleBackend` + `StubStorageBackend`. Workers get scoped AWS STS creds via `RequestCapability(type=STORAGE_MOUNT, scope={bucket, prefix, mode})`; daemon materializes `aws_*` files into the desk's secret dir with container-uid chown.
...
- **Resume regenerates overlay.** Overlay-code changes land on `drydock create <suspended-name>` without `--force`-destroying the worktree.
...
- **drydock daemon status under systemd.** Health-derives-from-socket, not pid-file.
...
- `firewall_extra_domains` YAML drift not reconciled on resume (needs `drydock project reload <name>`)
- `drydock overlay regenerate` as an explicit CLI command (today implicit via resume)
...
**Lifecycle**
- `drydock create / list / inspect / stop / destroy / attach / exec / status`
...
- Per-workspace secrets at `~/.drydock/secrets/<dock_id>/` mounted readonly at `/run/secrets/` in the container
...
**Integration**
- `drydock attach <name> [--editor <binary>]` opens Cursor / VS Code / code-insiders attached to the container at the right workspace folder via vscode-remote URI
...
- `devcontainer down` called in `drydock destroy` (no such subcommand; swapped to `docker stop` + `docker rm`)
