...
**Purpose.** Pin down the wire between clients (Harbor CLI, in-drydock CLI, later: mobile apps) and `wsd`. Names the methods, the transport, the auth model, and what happens on daemon restart.
...
**Decision.** JSON-RPC 2.0 over a Unix domain socket at `~/.drydock/wsd.sock` (mode `0600`, owner = Harbor user). Socket is bind-mounted into every drydock via the overlay so drydock-mode callers reach the daemon without networking.

**HTTP/Tailscale transport is deferred, not shipped in V2.** `v2-scope.md` originally proposed HTTP/Tailscale; in V2 (single-Harbor, single-daemon) the socket is simpler, cheaper, and removes an authentication surface that has no V2 caller. Wire protocol is identical either way; adding HTTP later reuses the same JSON-RPC envelope with bearer auth — useful if a thin multi-Harbor case ever surfaces (second Harbor runs its own daemon, `ws` CLI on either Harbor can target either daemon). Not a V2 goal.
...
RequestCapability(type, scope, ttl?)      → Lease         (subject = authenticated drydock from bearer token)
...
- **`CreateDesk` vs. `SpawnChild`** are intentionally distinct methods. `CreateDesk` is Harbor-authorised (no parent; caller is the Harbor user). `SpawnChild` is drydock-authorised (caller is a drydock with `spawn_children` capability; policy validator runs). Collapsing them would require every `CreateDesk` to pass a null-parent sentinel and the validator to special-case it. Splitting them up front keeps the trust boundary visible in the RPC surface.
...
- **`RequestCapability` does NOT take a `desk_id` argument.** The subject drydock is derived from the caller's bearer token via the auth middleware. This avoids a confused-deputy class of bugs where a compromised or buggy client could request leases targeted at another drydock. Admin impersonation (where an operator acts on behalf of a drydock) uses a separate `AdminRequestCapability(desk_id, ...)` method guarded by a Harbor-admin token, not the daemon's standard bearer auth — **deferred to V2.5 if ever needed**; no V2 callers today.
- **Read-only introspection (`ListDesks`, `InspectDesk`, `GetAudit`) can optionally bypass the daemon** when called by Harbor CLI — direct SQLite reads produce the same result. Drydock-mode clients always go through the daemon for narrowness.
...
|---|---|
| `CreateDesk`, `SpawnChild` | By `request_id`. Retry with same id → returns cached outcome. Retry with new id + same `(name, parent_id)` where drydock already exists → reject with `desk_exists`. |
| `StopDesk` | Naturally idempotent. Stopping a stopped drydock returns success. |
| `DestroyDesk` | Naturally idempotent. Destroying a gone drydock returns success. |
...
2. For each: reconcile against Docker + filesystem state.
   - Container running + matches spec → mark `completed`, register drydock if missing.
   - Container absent or partial → roll back (remove overlay file, delete registry row if present), mark `failed`.
3. Client-side: drydock-mode CLI retries the same `request_id`. Daemon returns the reconciled outcome. If the task failed, the client sees the failure. If it succeeded, the client sees the DeskRef.

**Why this matters.** Without this, a flaky network during `CreateDesk` can produce a phantom drydock: container exists, no registry row, client saw a timeout. Nested-spawn scenarios make this worse because the parent drydock's retry logic must be deterministic.
...
- Mounted into the drydock at `/run/secrets/drydock-token`, tmpfs, mode `0400`, owner container-uid.
- Daemon persists a SHA-256 of the token in SQLite (not the plaintext); the mounted file is the only copy. If the drydock's tmpfs is wiped, the drydock is unauthenticated until a rotation.
...
- Leaked token grants whatever the drydock currently has, nothing more.
...
- **mTLS via Tailscale device identity.** Strong but couples V2 to Tailscale beyond transport. If a drydock ever runs outside Tailscale (dev host without tailnet, CI), identity breaks.
- **Unix-socket peer credentials only.** Works single-Harbor-single-daemon. If a thin multi-Harbor case ever surfaces, peer creds don't travel, and bolting bearer tokens on then is more work than just shipping them now.

**Reversibility: MEDIUM.** Adding mTLS *on top of* bearer tokens is additive (stronger mutual authentication for specific callers). Removing bearer tokens in favor of something else is a breaking change for every drydock's `/run/secrets/drydock-token`.
...
**Survives reboot.** User-level services, not root. Starts on login. If the user isn't logged in, no daemon — fine for V2 (solo user). Promote to a system service if an always-on headless Harbor genuinely needs the daemon up regardless of login session.
...
**Decision.** `ws attach` stays direct — Harbor CLI opens an editor (VS Code, Cursor) connected to the drydock's container via the existing `vscode-remote+dev-container://` URL pattern. Daemon is NOT in the path.
...
**Caveat: in-drydock `ws attach` (drydock → other drydock).** Not supported in V2. If a use case emerges, it would need to go through the daemon (the source drydock is authenticated; the daemon validates that source can reach target). Defer until pressure exists.
...
```json
{
  ...
  "message": "Drydock 'myapp' is already running",
...
| `CreateDesk`/`SpawnChild` as distinct methods | Medium | Once clients, audit, and tests depend on the split (Harbor-admin vs. drydock-authenticated policy-validated authority), reshaping is expensive. Codex flagged the initial `Low` rating as understated |
